import { Component, ChangeDetectionStrategy, HostListener, OnInit, OnDestroy } from '@angular/core';
import { MatIconModule } from '@angular/material/icon';
import { CommonModule } from '@angular/common';
import { NotificationService } from './notification.service';
import { NgForOf } from '@angular/common';
import { Observable, Subject } from 'rxjs';
import { map, defaultIfEmpty, takeUntil } from 'rxjs/operators';
  ...
  template: `
    <span class="relative">
      ...
      <div *ngIf="isModalOpen" class="absolute right-0 mt-2 w-80 bg-[#313131] shadow-lg rounded-lg overflow-hidden z-50">
        <div class="p-4 border-[#212121]-700 flex justify-between items-center">
          <h2 class="text-[10px] font-semibold text-center">Notifications</h2>

          <!-- Mark All As Read Button -->
          <span *ngIf="unreadCount$ | async as unreadCount">
            <button *ngIf="unreadCount > 0" (click)="markAllAsRead()" class="text-xs text-gray-300 hover:text-gray-500">
              Mark All as Read
            </button>
          </span>
        </div>
